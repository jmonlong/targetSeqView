      ...
      if(length(rngsAlign)>1 & length(fordPEs)>1){
        ...
        while(all(difrefs == FALSE) & splitlim>0){
          difrefs <- sapply(fordPEs,function(x){
            ...
            if((userec==FALSE | findSplitReads == FALSE) & length(x[[1]])>0 & length(x[[2]])>0){ ## JM: This fix is not optimal. The bug might be something wrong with the production of the fordPEs...
